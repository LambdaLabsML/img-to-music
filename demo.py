...
def get_prompts(uploaded_image, track_duration, gen_intensity, gen_mode):
    ...
    print(prompt)
    music_result = generate_track_by_prompt(
        prompt, track_duration, gen_intensity, gen_mode
    )
    print(music_result)
    return (
        music_result[0],
...
with gr.Blocks(css="style.css") as demo:
    with gr.Column(elem_id="col-container"):
        ...
        music_output = gr.Audio(
            label="Result", type="filepath", elem_id="music-output"
        ).style(height="5rem")
        ...
        with gr.Accordion(label="Music Generation Options", open=False):
            ...
            with gr.Row():
                gen_intensity = gr.Dropdown(
                    choices=["low", "medium", "high"], value="medium", label="Intensity"
                )
                gen_mode = gr.Radio(
                    label="mode", choices=["track", "loop"], value="track"
                )
    ...
    generate.click(
        get_prompts,
        inputs=[input_img, track_duration, gen_intensity, gen_mode],
        outputs=[music_output, share_button, community_icon, loading_icon],
        api_name="i2m",
    )
